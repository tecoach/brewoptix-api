...
python check_script_number_in_deployment_table.py $STAGE 32
if [ $? = 1 ]; then
...
cd $PWD/deployment_scripts/32_BREWAPI_192 || exit
...
cd $PWD/deployment_scripts/common || exit
python add_script_number_to_deployment_table.py $STAGE 32
