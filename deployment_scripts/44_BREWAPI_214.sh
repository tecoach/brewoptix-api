...
python check_script_number_in_deployment_table.py $STAGE 44
if [ $? = 1 ]; then
...
echo "Deploy counts service"
cd $PWD/services/counts || exit
...
cd $PWD/deployment_scripts/common || exit
python add_script_number_to_deployment_table.py $STAGE 44
