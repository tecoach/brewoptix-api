...
python check_script_number_in_deployment_table.py $STAGE 41
if [ $? = 1 ]; then
...
echo "Deploy purchase_order_email service"
...
cd $PWD/deployment_scripts/common || exit
python add_script_number_to_deployment_table.py $STAGE 41
